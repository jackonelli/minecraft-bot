    ...
    for state, action, reward, next_state, done in data.batch_iter(batch_size=1, num_epochs=500, seq_len=2000):
        ...
        for i in range(batch_size):
            for j in range(length):
                ...
                camera_thresholds = (abs(action['camera'][i][j][0]) + abs(action['camera'][i][j][1])) / 2.0
                if (camera_thresholds > 2.5):
                    if ( (action['camera'][i][j][1] < 0) & ( abs(action['camera'][i][j][0]) < abs(action['camera'][i][j][1]) ) ):
                        if (action['attack'][i][j] == 0):
                            action_index = 0
                        else:
                            action_index = 1
                    elif ( (action['camera'][i][j][1] > 0) & ( abs(action['camera'][i][j][0]) < abs(action['camera'][i][j][1]) ) ):
                        if (action['attack'][i][j] == 0):
                            action_index = 2
                        else:
                            action_index = 3
                    elif ( (action['camera'][i][j][0] < 0) & ( abs(action['camera'][i][j][0]) > abs(action['camera'][i][j][1]) ) ):
                        if (action['attack'][i][j] == 0):
                            action_index = 4
                        else:
                            action_index = 5
...
class Qnetwork():
    def __init__(self):
        ...
        self.input_img_dq = tf.compat.v1.placeholder(shape=[None,64,64,3], dtype=tf.float32)
        self.scale_img_dq = tf.map_fn(image_scale, self.input_img_dq, dtype=tf.float32)

        self.input_img_nstep = tf.compat.v1.placeholder(shape=[None,64,64,3], dtype=tf.float32)
        self.scale_img_nstep = tf.map_fn(image_scale, self.input_img_nstep, dtype=tf.float32)

        self.input_expert_action = tf.compat.v1.placeholder(shape=(None,2), dtype=tf.int32)
        self.input_is_expert = tf.compat.v1.placeholder(shape=[None,1], dtype=tf.float32)
        self.input_expert_margin = tf.compat.v1.placeholder(shape=[None,action_len], dtype=tf.float32)

        self.conv1_dq = tf.compat.v1.layers.conv2d(inputs=self.scale_img_dq, filters=32, kernel_size=[8,8], strides=[4,4],
                                         padding='VALID', activation=tf.nn.relu,
                                         kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")),
                                         kernel_regularizer=tf.keras.regularizers.l2(l=0.5 * (10e-5)),
                                         bias_regularizer=tf.keras.regularizers.l2(l=0.5 * (10e-5)))
        self.conv2_dq = tf.compat.v1.layers.conv2d(inputs=self.conv1_dq, filters=64, kernel_size=[4,4], strides=[2,2],
                                         padding='VALID', activation=tf.nn.relu,
                                         kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")),
                                         kernel_regularizer=tf.keras.regularizers.l2(l=0.5 * (10e-5)),
                                         bias_regularizer=tf.keras.regularizers.l2(l=0.5 * (10e-5)))
        self.conv3_dq = tf.compat.v1.layers.conv2d(inputs=self.conv2_dq, filters=64, kernel_size=[3,3], strides=[1,1],
                                         padding='VALID', activation=tf.nn.relu,
                                         kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")),
                                         kernel_regularizer=tf.keras.regularizers.l2(l=0.5 * (10e-5)),
                                         bias_regularizer=tf.keras.regularizers.l2(l=0.5 * (10e-5)))
        self.x_dq = tf.compat.v1.layers.flatten(self.conv3_dq)
        self.dq_output = tf.compat.v1.layers.dense(self.x_dq, action_len, activation=tf.nn.relu)

        self.conv1_nstep = tf.compat.v1.layers.conv2d(inputs=self.scale_img_nstep, filters=32, kernel_size=[8,8], strides=[4,4],
                                            padding='VALID', activation=tf.nn.relu,
                                            kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")),
                                            kernel_regularizer=tf.keras.regularizers.l2(l=0.5 * (10e-5)),
                                            bias_regularizer=tf.keras.regularizers.l2(l=0.5 * (10e-5)))
        self.conv2_nstep = tf.compat.v1.layers.conv2d(inputs=self.conv1_nstep, filters=64, kernel_size=[4,4], strides=[2,2],
                                            padding='VALID', activation=tf.nn.relu,
                                            kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")),
                                            kernel_regularizer=tf.keras.regularizers.l2(l=0.5 * (10e-5)),
                                            bias_regularizer=tf.keras.regularizers.l2(l=0.5 * (10e-5)))
        self.conv3_nstep = tf.compat.v1.layers.conv2d(inputs=self.conv2_nstep, filters=64, kernel_size=[3,3], strides=[1,1],
                                            padding='VALID', activation=tf.nn.relu,
                                            kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")),
                                            kernel_regularizer=tf.keras.regularizers.l2(l=0.5 * (10e-5)),
                                            bias_regularizer=tf.keras.regularizers.l2(l=0.5 * (10e-5)))
        self.x_nstep = tf.compat.v1.layers.flatten(self.conv3_nstep)
        self.nstep_output = tf.compat.v1.layers.dense(self.x_nstep, action_len, activation=tf.nn.relu)

        self.targetQ_dq = tf.compat.v1.placeholder(shape=[None], dtype=tf.float32)
        self.targetQ_nstep = tf.compat.v1.placeholder(shape=[None], dtype=tf.float32)
        ...
        self.Q_dq = tf.reduce_sum(input_tensor=tf.multiply(self.dq_output, self.actions_onehot), axis=1)
        self.Q_nstep = tf.reduce_sum(input_tensor=tf.multiply(self.nstep_output, self.actions_onehot), axis=1)

        self.slmc = tf.reduce_mean(input_tensor=tf.abs(self.slmc_output))
        self.td_error_dq = tf.reduce_mean(input_tensor=tf.square(self.targetQ_dq - self.Q_dq))
        self.td_error_nstep = tf.reduce_mean(input_tensor=tf.square(self.targetQ_nstep - self.Q_nstep))

        self.loss = tf.reduce_sum(input_tensor=self.td_error_dq + self.td_error_nstep + self.slmc)
...
def main():
    ...
    data_path = os.path.join(root_path, 'data')  # Or wherever you have stored the expert data
